<details>
  <summary>
    電力市場価格データ配信のパフォーマンス改善
    <span>2024年/6ヶ月</span>
  </summary>
  <div>
    <ul>
      <li><strong>カテゴリ:</strong> <span>Webサービス</span> <span>自社プロダクト</span> </li>
      <li><strong>担当工程:</strong> <span>コーディング</span> <span>テスト</span> <span>運用・保守</span> <span>その他</span> </li>
      <li><strong>職種・役割:</strong> <span>バックエンド</span> <span>インフラ</span> </li>
      <li><strong>使用技術:</strong> <span>Python</span> <span>AWS</span> <span>AWS Lambda</span> <span>AWS Step Function</span> <span>Amazon S3</span> <span>Amazon Redshift</span> <span>CI/CD</span> <span>Git</span> <span>Glue</span> <span>Athena</span> <span>Cloud Formation</span> <span>Tableau</span> </li>
    </ul>
  </div>
...
電力市場価格データを扱うシステムのデータ処理パフォーマンス最適化を目的として、CSVからParquet形式への移行、GlueとRedshiftの最適化、Redshiftノード変更、Tableauのデータ抽出方法改善を実施。データ加工・変換部分とGlueによるRedshiftへのデータ投入を担当。

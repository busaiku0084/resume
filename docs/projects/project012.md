<details>
  ...
  <div>
    <ul>
      <li><strong>カテゴリ:</strong> <span>自社プロダクト</span> </li>
      <li><strong>担当工程:</strong> <span>要件定義</span> <span>設計</span> <span>コーディング</span> <span>テスト</span> <span>運用・保守</span> <span>その他</span> </li>
...
新規に収集した外部データを基に、BIツール（Tableau）での可視化を目的としたデータパイプラインを構築。
データの取得元はWebページやPDFファイルが中心であり、スクレイピングやPDF解析を行いながら、GlueとRedshiftを組み合わせて加工・蓄積。
分析用データはParquet形式で管理し、パフォーマンスを重視した構成とした。
